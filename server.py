...
async def rate_limit_middleware(request: _Request, call_next):
    ...
    # HTML 파일 캐시 방지 (브라우저 구버전 캐시 문제 예방)
...
""".strip()
    ...
    SECTIONS_ALL = {
        1: "상황 분석 요약 (이 상황의 핵심 코칭 포인트 5~6개)",
        2: "핵심 코칭 표현 (이 상황에서 가장 효과적인 표현 5~6개)",
        3: "절대 금기 표현 (이 상황에서 써선 안 되는 표현 5~6개 — 짧은 이유 포함)",
        4: "대화 오프닝 스크립트 (이 상황에 맞는 시작 문장 5~6개)",
        5: "완성형 피드백 문장 예시 (이 맥락이 담긴 완전한 문장 5~6개)",
        6: "성찰 유도 코칭 질문 (이 상황에 맞는 열린 질문 5~6개)",
        7: "마무리 · 동기부여 표현 (다음 행동으로 연결하는 마무리 5~6개)",
        8: "부드럽게 전달하는 대화 Tip (톤·타이밍·바디랭귀지·MBTI 적용 포인트 5~6개 — 각 항목 한 줄로)",
    }
...
위 정보를 분석하여 다음 섹션들을 한국어로 작성해 주세요. **각 섹션은 5~6개 항목으로 균일하게 맞추고 그 이상은 작성하지 마세요.** 각 항목은 불릿(·)으로 구분하고 이 상황의 맥락과 뉘앙스를 충분히 반영하되, 항목 자체는 1~2줄로 간결하게 써 주세요.
