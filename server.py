...
아래 조건에 맞는 코칭 Vocabulary Pool을 생성해 주세요.
...
다음 7개 섹션을 작성해 주세요. **각 섹션은 5~6개 항목으로 균일하게 맞추고 그 이상은 작성하지 마세요.** 각 항목은 불릿(·)으로 구분하고, 항목 자체는 1~2줄로 간결하게 써 주세요.

## 1. 핵심 코칭 표현 (이 유형 및 상황에서 가장 효과적인 표현 5~6개)
## 2. 절대 금기 표현 (써선 안 되는 표현 5~6개 — 짧은 이유 포함)
## 3. 대화 오프닝 (다양한 방식의 오프닝 5~6개)
## 4. 완성형 피드백 문장 예시 (구체적 맥락이 담긴 완전한 문장 5~6개)
## 5. 성찰 유도 코칭 질문 (열린 질문 형식 5~6개)
## 6. 마무리 표현 (동기부여 및 다음 행동 연결 5~6개)
## 7. 감정 인식 및 공감 표현 (5~6개)

모든 표현은 {req.emp_type} 유형의 특성을 반영해 주세요. 마지막 7번 섹션의 마지막 항목까지 반드시 끝맺어 주세요."""
